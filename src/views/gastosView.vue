        ...
        <v-card class="ma-3">
            <v-row class="d-flex  px-6 my-4">
                <v-col lg="9" md="9" sm="8" style="min-width: 200px;">
                    <div class="d-flex align-center "><v-icon size="x-large" icon="mdi-cash"></v-icon>
...
export default {
    ...
    data: () => ({
        ...
        gastoRules: [v => !!v || 'El monto es requerido', v => (v && /^[0-9]+$/.test(v)) || 'El numero no debe contener caracteres'],
        categorias: ["Servicios Públicos", "Compra de productos e insumos", "Arriendo", "Nómina", "Gastos administrativos", "Mercadeo y publicidad", "Transporte, domicilios y logísica", "Mantenimiento y reparaciones", "Muebles, equipos o maquinaria", "Otros"],
        dialogG: false,
        gastos: [],
    }),
    methods: {
        async crearGasto() {
            ...
            axios.post(`${process.env.VUE_APP_API_URL}/gasto/crear`, this.formGasto).then(async (resp) => {
                if (resp.status == 201) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Exitoso',
                ...
                return Swal.fire({ icon: 'error', title: 'No se pudo crear el gasto', timer: 1000 });
            })
        },
        async limpiarForm() {
            this.dialogG = false;
        ...
        async eliminarGasto(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    await axios.delete(`${process.env.VUE_APP_API_URL}/gasto/${id}`).then(async () => {
